...
# Author: M. Keefe <mdkeef25@g.holycross.edu>
...
# global variables for server data
topic_list_version = 0

# protects the global topics list
updates = threading.Condition()

# initializes some default topics (whatever, holycross, music)
# and variables for each topic (version number, mention count, like count, messages)
topics = {
    "whatever": {"version": 0, "mention_count": 0, "like_count": 0, "messages": []},
    "holycross": {"version": 0, "mention_count": 0, "like_count": 0, "messages": []},
    "music": {"version": 0, "mention_count": 0, "like_count": 0, "messages": []}
}
...
def handle_http_get_hello(req, conn):
    ...
    <html>
        ...
        <body>
            ...
            <p>You have visited this page an unknown numbrer of times before.</p>
            ...
            <p>http://{server_host}:{server_port}/hello        - this page</p>
            <p>http://{server_host}:{server_port}/status       - status and statistics</p>
            <p>http://{server_host}:{server_port}/quote        - a random quote</p>
            <p>http://{server_host}:{server_port}/welcome.html - a welcome page</p>
            <p>http://{server_host}:{server_port}/index.html   - some fancy web page</p>
            <p>Enter your name: ___________</p>
            <p>Favorite color: ___________</p>
            <p>Just kidding, this page isn't yet interactive. Sorry.</p>
        </body>
    </html>
    """.format(name=name, favcolor=favcolor, time=time.strftime("%a, %d %b %Y %H:%M:%S %Z"), 
...
# GET request handler for topics list with version checking
def handle_http_get_topics(req):
    request_parameters = req.path.split("?")[1] if "?" in req.path else ""
    version_str = request_parameters.split("=")[1] if "version=" in request_parameters else "0"

    try:
        version = int(version_str)
    except ValueError:
        return Response("400 Bad Request", "text/plain", "Invalid version number")

    with updates:
        # handles version 0: send the current list of topics and version
        if version == 0:
            return build_topic_response()

        # handles version 1 or higher: enter an infinite loop, simulating the wait
        while True:
            pass  # does nothing and simulates waiting forever

# builds the response for the topics list in the required format
def build_topic_response():
    with updates:
        response_body = f"{topic_list_version}\n"
        for topic_name, topic_data in topics.items():
            response_body += f"{topic_data['mention_count']} {topic_data['like_count']} {topic_name}\n"
        return Response("200 OK", "text/plain", response_body)

# POST request handler for new messages
def post_message(new_topic_name, message_text):
    global topic_list_version
    with updates:
        if new_topic_name not in topics:
            
            # adds a new topic if it doesn't exist
            print(f"Creating new topic: {new_topic_name}")
            topics[new_topic_name] = {"mention_count": 1, "like_count": 0, "messages": [message_text], "version": 1}
        else:

            # if the topic exists, update the mention count, add the message, and increment the version
            topics[new_topic_name]["mention_count"] += 1
            topics[new_topic_name]["messages"].append(message_text)
            topics[new_topic_name]["version"] += 1  # Increment the topic-specific version

        # increments the global version **after** the topic-specific version
        topic_list_version += 1

        print(f"Global Version Incremented: {topic_list_version}, {new_topic_name} Version: {topics[new_topic_name]['version']}")

        # notifies all clients of the update
        updates.notify_all()


# GET request handler for specific topic feed with version checking
def handle_http_get_feed(req):
    path_prefix = "/whisper/feed/"
    if not req.path.startswith(path_prefix):
        return Response("400 Bad Request", "text/plain", "Invalid feed request format")

    topic_name = req.path[len(path_prefix):].split("?")[0]
    version_str = req.path.split("?")[1].split("=")[1] if "version=" in req.path else "0"
    ...
    with updates:
        if topic_name not in topics:
            return Response("404 Not Found", "text/plain", f"Topic '{topic_name}' not found")

        topic_version = topics[topic_name]["version"]

        # waits until the topic's version is at least the requested version
        while version >= topic_version:
            if not updates.wait(timeout=30):
                return Response("408 Request Timeout", "text/plain", "No updates available within the timeout period")
            topic_version = topics[topic_name]["version"]

        # builds the response body
        response_body = f"{topic_version}\n"
        for message in topics[topic_name]["messages"]:
...
def handle_http_post_message(req, conn):
    ...
    with updates:
        try:
            ...
            # error check if not the expected two-line format
            if len(lines) != 2:
                return Response("400 Bad Request", "text/plain", "expected two lines in the request body")

            # extracts tags and message
            tags_line = lines[0]
            message_line = lines[1]

            if not tags_line.startswith("tags... ") or not message_line.startswith("message... "):
                return Response("400 Bad Request", "text/plain", "request must start with 'tags... ' and 'message... '")

            # extracts the topics and message text
            topics_list = tags_line[8:].strip().split()  # skips "tags... "
            message_text = message_line[11:].strip()  # skips "message... "

            if not message_text:
                return Response("200 OK", "text/plain", "empty messages are ignored")

            if not topics_list:
                topics_list = ['whatever']

            # updates the topics list for each mentioned topic
            for topic in topics_list:
                post_message(topic, message_text)

            return Response("200 OK", "text/plain", "success")

        except Exception as e:
            return Response("500 Internal Server Error", "text/plain", str(e))

# POST request handler for likes
def handle_http_post_likes(req, conn):
    global topic_list_version

    try:
        # extracts the topic from the path
        path_parts = req.path.split("/")
        if len(path_parts) != 4 or not path_parts[3]:
            return Response("400 Bad Request", "text/plain", "Invalid topic specified")

        topic = path_parts[3]  # The topic (ex: "holycross", "whatever")

        # thread-safe update using the condition variable
        with updates:
            if topic in topics:
                # increments both the like count and topic-specific version
                topics[topic]['like_count'] += 1
                topics[topic]['version'] += 1 

                # increments the global version
                topic_list_version += 1

                # logs the increments for debugging purposes
                print(f"Global Version Incremented: {topic_list_version}, {topic} Version: {topics[topic]['version']}")

                # notifies all threads waiting for an update
                updates.notify_all()
            else:
...
def handle_http_post(req, conn):
    if req.path == "/whisper/messages":
        ...
        resp = Response("404 Not Found", "text/plain", "invalid path")
    return resp
